...
class Sidebar(ctk.CTkScrollableFrame):
    ...
    def _build(self):
        ...
        # ── PASSO 1 — Tipo ──
        self._label("Passo 1 — O que você quer lançar?")
        self._label("  Dados: gera somas aleatórias por lançamento", dim=True)
        self._label("  Moedas: conta quantas caras saem por rodada", dim=True)
        ...
        self._label("  O valor selecionado aparece em destaque abaixo", dim=True)
        ...
        self._label("  d4=4 faces  d6=6 faces  d8=8 faces  d20=20 faces", dim=True)
        self._label("  Ignore este passo se usar moedas", dim=True)
        ...
        # ── PASSO 4 — Simulações ──
        self._label("Passo 4 — Quantas simulações executar?")
        self._label("  Mais simulações = resultado mais fiel à teoria", dim=True)
        self._label("  Recomendado: pelo menos 1.000", dim=True)
        ...
        self._label("  Ou digite um valor personalizado:", dim=True)
        ...
        self._label("Teórico = calculado pela fórmula matemática", dim=True)
        self._label("Experim. = obtido nas simulações aleatórias", dim=True)
        self._label("✓ desvio < 1%  |  ⚠ desvio > 5%", dim=True)
